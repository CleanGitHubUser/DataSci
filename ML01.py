...
# 기존의 모델이나 결과물을 개선하거나 예측 하게끔
# 구축하는 과정을 의미

# 데이터를 이용해서 명시적으로 정의되지 않은 패턴을
# 컴퓨터로 학습하여 결과를 만들어내는 분야
...
#          데이터의 양에 의존하는 기법으로 다양한 패턴과 경우애
#          유연하게 대응하는 구조로 만들어 많은 데이터를 이용해서
#          학습해야 성능이 향상되는 구조 채택

# 머신러닝 기반 데이터 분석 계획 및 절차

# 비즈니스 이해 및 문제 정의 - 분석주제 선정,
# 데이터 수집 - 데이터 확보
# 데이터 전처리와 탐색 - 결측치/이상치 처리, 변환, 표준화, 요약, 시각화
# 데이터에 대한 모델 훈련 - train/test 데이터 분리, 머신러닝, 기법 적용
# 모델 성능 평가 - 평가지표 계산
...
# 데이터 분할 방법 1 :
# iris_train <- iris[1:105, ] # 1 ~ 105행까지
# iris_test <- iris[106:150, ] # 106 ~ 나머지
...
print(iris['target_names']) # iris 타겟(품종) 처음 5행만 출력
...
# k-최근접 알고리즘에서는 k는 가장 가까운 이웃 '하나'가 아니라
# 훈련데이터 중 새로운 데이터와
# 가장 가까운 k개의 이웃을 찾는다는 의미
# KNN 알고리즘 scikit-learn 의 Estimaor 클래스에 구현
# KNN 알고리즘은 neighbors 모듈의 KNeighborClassifier 클래스에 구현
...
# 이진 분류 : 2가지 질문의 답 중 하나를 에측 (스팸메일 분류)
# 다중 분류 : 3가지 이상 질문의 답 중 하나를 예측

# 일반화/과대적합/과소적합
# 일반화: 모델을 통해 처음 보는 데이터에 대해
#         정확히 예측할 수 있는 경우
