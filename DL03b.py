...
# 이미지 파일경로 정의
find_dir = 'D:/빅데이터/data/pdf/빅데이터/데이터분석통계/data/딥러닝/101_ObjectCategories'
cache_dir = 'D:/빅데이터/data/pdf/빅데이터/데이터분석통계/data/딥러닝/101_ObjectCategories/cache_avghash'

# average hash cache 디렉토리가 없으면 자동 생성
if not os.path.exists( cache_dir ) :
    os.mkdir(cache_dir)

# 이미지를 평균해쉬법에 의해 픽셀로 변환
# avghash 파일에 대한 cache 파일을 생성
...
# 실제 찾을 이미지파일 지정
# srcfile = find_dir + "\octopus\image_0007.jpg" # 이미지
srcfile = "data/image_0014.jpg" # 이미지
html = ""
sim = list(find_image(srcfile, 0.20))
sim = sorted(sim, key=lambda x: x[0])
# 찾은 이미지들은 html 형태로 출력
for r, f in sim:
    print(r, ">", f)
